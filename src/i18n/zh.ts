...
export const zh = {
  ...
  'style.effectOutManaged': '翻转和上移会自行管理字幕之间的切换，因此这些字幕行不能再叠加独立退场特效或退场时长。',
  'style.effectDuration': '特效时长（秒）',
  'style.effectDurationHint': '后修改的 In/Out 优先；另一侧会自动缩短，不超出 segment 且不重叠。',
  'style.effectDurationNoneHint': '“无”不包含动画，因此时长不适用。',
  ...
  'tl.cantFreeze': '播放头不在该视频线段内部，无法定格画面',
  'tl.noAudioStream': '该视频不包含音频流',
  'tl.splitTitle': '在红色播放头位置把线段切成两段',
  'tl.splitWord': '切割',
  'tl.freezeTitle': '在红色播放头位置切开，并插入该精确画面的 3 秒定格片段',
  'tl.freezeFrame': '定格画面（3秒）',
  'tl.freezeBadge': '定格 {n}秒',
  'tl.start': '开始',
  'tl.loop': '循环',
  'tl.times': '次',
