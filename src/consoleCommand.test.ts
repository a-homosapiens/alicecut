...
describe('caption selection console commands', () => {
  ...
  it('selects every caption and batch-applies in/out effects and style', async () => {
    ...
    expect(captions.every((line) => line.effectId === 'wave-in')).toBe(true)
    expect(captions.every((line) => line.effectOutId === 'evaporate-out')).toBe(true)
  ...
  it('reports a rejected Out effect instead of silently claiming it was applied', async () => {
    const logs: string[] = []
    await runConsoleCommand(
      JSON.stringify({ select: 'captions', effectIn: 'rise', effectOut: 'pop' }),
      (message) => logs.push(message)
    )

    const captions = useProject.getState().lines.filter((line) => line.kind !== 'text')
    expect(captions.every((line) => line.effectId === 'rise' && line.effectOutId == null)).toBe(true)
    expect(logs).toContain(
      '✗ effectOut: the chosen effect is entrance-only or the selected Flip/Rise captions manage their own transition'
    )
  })
})
