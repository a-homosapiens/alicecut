...
- **8 种特效**：逐字弹出 / 缩放冲击 / 滑动错落 / 打字机 / 发光渐显，以及整句停靠转场——翻转切换、翻转·底对齐、上移切换（旧句不消失，停靠在新句侧边/上方）
...
- **字幕与独立文字**：播放头处一键加一句字幕 / 删除选中字幕；可加**独立文字块**（标题/水印），起止自由拖动，特效与字幕共用全部 8 种
- **画布内直接拖动**调整每句歌词的画面位置
...
- **字幕样式**：字幕底色块（颜色 + 透明度）、文字透明度、粗体/斜体、光晕、阴影（颜色/偏移/模糊），GUI 与 job.json 均可设置
- 内置 13 款免费商用中文字体（得意黑、霞鹜文楷、庞门正道系列、站酷系列、江西拙楷、锐字真言体），支持导入任意 ttf/otf；工程保存/载入
